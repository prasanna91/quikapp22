...
inject_info_plist() {
    ...
    add_info_plist_key "$info_plist" "CFBundleExecutable" "\$(EXECUTABLE_NAME)"
...
EOF
    ...
    # === Configuration ===
      ...
      echo "📦 Backup created: ${PBXPROJ_PATH}.backup"

      # --- Automatic Code Signing ---
      # Remove any manual signing settings to enforce automatic signing
      # Remove PROVISIONING_PROFILE_SPECIFIER and CODE_SIGN_IDENTITY from project.pbxproj
      sed -i '' "/PROVISIONING_PROFILE_SPECIFIER = /d" "$PBXPROJ_PATH"
      sed -i '' "/CODE_SIGN_IDENTITY = /d" "$PBXPROJ_PATH"

      # Remove all possible manual signing keys to enforce automatic signing
      sed -i '' '/CODE_SIGN_IDENTITY/d' "$PBXPROJ_PATH"
      sed -i '' '/CODE_SIGNING_REQUIRED/d' "$PBXPROJ_PATH"
      sed -i '' '/CODE_SIGNING_ALLOWED/d' "$PBXPROJ_PATH"
      sed -i '' '/PROVISIONING_PROFILE_SPECIFIER/d' "$PBXPROJ_PATH"
      ...
      # === Update PRODUCT_NAME ===
      sed -i '' "s/PRODUCT_NAME = .*;/PRODUCT_NAME = \"$app_name\";/g" "$PBXPROJ_PATH"

      # Ensure ProvisioningStyle is set to Automatic for all targets
      sed -i '' 's/ProvisioningStyle = Manual;/ProvisioningStyle = Automatic;/g' "$PBXPROJ_PATH"

      echo "✅ project.pbxproj updated with:"
      echo "   - Bundle ID: $bundle_id"
      echo "   - Product Name: $app_name"
      echo "   - Removed manual signing keys (if any)"
}
